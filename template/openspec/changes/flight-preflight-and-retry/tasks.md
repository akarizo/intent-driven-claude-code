...
- [x] S4 `pr-ship.md` 唯一一次问询前移到 step 10 自动修复前（默认只贴评论），step 11 不再问；`opsx-propose.md` / `openspec-propose/SKILL.md` step 4 基线红即停下报告；`schema.yaml` Slicing rules 加「verify 只跑测试」；scenarios：pr-ship-asks-before-autofix · propose-baseline-red-stops · schema-verify-excludes-typecheck
