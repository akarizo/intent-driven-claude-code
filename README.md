...
your-project/
├── .claude/
│   ├── commands/       # 12 个 slash 命令（见下方）
│   └── skills/         # 15 个 skill（见下方）
├── openspec/
...
| `/opsx-propose <name>` | 一次性生成 apply 所需的全部 artifacts |
...
## CLAUDE.md.snippet（10 行硬约束）

`install.sh` 注入到目标项目 CLAUDE.md 末尾的 10 行常驻约束（用 marker 包裹幂等追加）：
