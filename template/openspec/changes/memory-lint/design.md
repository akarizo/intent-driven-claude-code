...
```
常驻索引   amc-afa 24094 B（仅 MEMORY.md）/ 221 实体文件  ← 唯一大项目
           第二名 1738 B / 10 files;其余 < 1 KB;5 个 memory 为空
二级索引   index-shipped-changes 5981 B + index-dlib-parser-facts 5026 B
           —— 按需 recall，不计入常驻
索引行     n=218  p50=147  p75=194  p90=335  p95=400  p99=511  max=882
           > 400B 10 条(4.6%) · > 500B 4 条 · > 700B 1 条(0.5%)
缺陷       amc-afa 1 悬空 + 1 孤儿 + 1 状态漂移(同一次重命名的三个面)
```

> ⚠ **口径修正（2026-08-30）**：立项调研时用 `startswith("- [")` 统计，漏掉了 `- 二级索引：[…](x.md)` 这类「文字在前、指针在后」的行，得出 `n=209 / max=552 / 700B 零触发`。实现后用 lint 自身的口径（`^\s*-\s` + 含指针，按行去重）复测，真实值是 `n=218 / max=882 / 700B 触发 1 条`。**闸的阈值不变**——882 B 那条确实该拦；改的只是文档里对分布的陈述。
...
| 闸 | 阈值 | 依据 |
|---|---|---|
| 索引单行 warn | > 400 B | 恰在 p95（218 条中 10 条超线，4.6%） |
| 索引单行 ERROR | > 700 B | 实测触发 1 条（882 B，`index-shipped-changes.md:32`）——确实该拦 |
| 常驻索引总量 warn | > 28 KB | `amc-afa` 24094 B 是唯一接近者，留 ~15% 余量；超线的正解是下沉二级索引而非删知识 |

**总量只算一级索引 `MEMORY.md`，二级索引不计入。** 二级索引按需 recall、本就不常驻；把它算进来会**惩罚「下沉」这个正确行为**——下沉后总字节不变，warn 永远消不掉，用户只会困惑「我照做了为什么还报」。实测印证：`amc-afa` 一级 24094 B 合格，若把两个二级索引（11007 B）算进去则是 35101 B、误报超线。
...
1. 现有项目首次触发时会报出存量问题。实测（2026-08-30，16 个有索引的项目）：**15 个零 ERROR 零 warn**；`amc-afa` 1 ERROR（882 B 超长索引行）+ 15 warn（9 条 400–552 B 行长 · 6 条真实状态漂移）。**双向闭合闸零误报**——立项调研中手工修复的那处已闭合，其余 217 个文件全部正常。存量 warn 由人按节奏消化，不阻断任何写入。
